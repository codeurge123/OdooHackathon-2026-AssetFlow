...
function AssetsPage({ data, runAction }) {
  ...
  return (
    <Panel title="Asset Registration & Directory">
      <form className="mb-5 grid gap-3 lg:grid-cols-[1fr_150px_150px_1fr_140px]" onSubmit={handleSubmit}>
      ...
      <div className="overflow-hidden rounded-lg border border-slate-200">
        <table className="w-full min-w-[760px] text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
            <tr>
              {['Tag', 'Name', 'Category', 'Status', 'Location', 'Action'].map((heading) => <th className="px-4 py-3" key={heading}>{heading}</th>)}
            </tr>
          </thead>
